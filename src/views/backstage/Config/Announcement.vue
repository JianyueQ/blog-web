    ...
    <div class="pro-card">
      <div class="card-body">
        ...
        <div class="md-editor-container" :class="{ 'preview-only': previewMode === 'preview' }">
          ...
          <div v-show="previewMode !== 'edit'" class="preview-pane">
            <div class="preview-header">
              <span>预览</span>
              <span v-if="updateTime" class="update-time">最后更新: {{ updateTime }}</span>
            </div>
        ...
        <div class="form-actions mt-20">
          ...
          <el-button size="large" round @click="handleClear">
            <el-icon><Delete /></el-icon>
            清空内容
          </el-button>
          <span v-if="saveTime" class="save-time">保存于: {{ saveTime }}</span>
        </div>
      </div>
    </div>
  </div>
